...
window.FCShapeUtils = (function () {
    ...
    var loadMesh = function (url, binaryMode) {
        ...
        var loadMeshWithWorker = function (meshUrl) {
            return new Promise(function (resolve, reject) {
                ...
                worker.postMessage({op:'load_mesh', src:meshUrl, mode:binaryMode && 'binary' || 'ascii'});
...
window.FCShapes = (function () {
    ...
    LatheShape.prototype.divulge = function () {
        ...
        var segmentHeight = lathe.height / (lathe.profile.length-1);
    
        var segPoly = new P.Poly();
        for (var i=0; i<(lathe.profile.length-1); i++) {
        
            var ylo = segmentHeight * i, yhi = segmentHeight * (i+1);
            var anglePer = (2*Math.PI)/this.segmentCount;
            var r1 = lathe.profile[i];
            var r2 = lathe.profile[i+1] || 0;
