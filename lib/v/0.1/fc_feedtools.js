...
var _looksyplugin = function () {
    return {
        name: 'Looksy',
        detect: function (url) {
            return url.includes('//meta4vr.net/') || url.includes('//looksy.meta4vr.net/') || url.includes('//vr.io.codex.cx/') || url.startsWith('/');
        },
        fetch: function (url) {
            ...
            if (dat.groups) {
                for (var i=0; i<dat.groups.length; i++) {
                    ...
                    boardsDat.push({
                        type: 'image',
                        src: myGroup.cover,
                        caption: myGroup.caption || myGroup.title || 'SET', /* not supported yet */
            ...
            if (dat.pics) {
                for (var i=0; i<dat.pics.length; i++) {
                    ...
                    boardsDat.push({
                        ...
                        meta_orig: myPic,
                        dest: null,
                        description: myPic.meta && (myPic.meta.description || myPic.meta.title || myPic.meta.rel || myPic.meta.filename),
                        descriptionBlocks: myPic.meta && [
                            {t:myPic.meta.title, size: '40', color:'black'},
                            {t:myPic.meta.author, size: '30', color:'black'}
                        ] || null
                    });
                }
            }
            ...
            for (var i=0; i<posts.length; i++) {
                if (posts[i].photos) {
                    ...
                    for (var j=0; j<p.photos.length; j++) {
                        ...
                        boardsdat.push({
                            type: 'image',
                            src: 'https://crossorigin.me/'+ph.url,
                            meta_orig: p,
...
window.FCFeedTools = (function () {
    ...
    function CylinderArranger(params) {
        ...
        this.baseOffset = params.baseOffset || 0.0; // -2.5
    ...
    CylinderArranger.prototype.arrange = function (rangeStart, rangeEnd) {
        ...
        for (var i=rangeStart; i<rangeEnd; i++) {
            ...
            var boardPos = {
                ...
                y:this.baseOffset + (this.rowHeight*currentRow)
            };
    ...
    function ContentWrangler(scene, params) {
        ...
        this.contentPostProcessor = null;
    ...
    ContentWrangler.prototype.loadContent = function (url, params, append) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            .then(function (dat) {
                var newContent;
                if (dat.meta && dat.meta.scene && wrangler.sceneUpdateCallback) {
                    console.log('Updating scene via callback');
                    wrangler.sceneUpdateCallback(dat.meta.scene, scene);
                }
                ...
                .then(function (boardsList) {
                    if (wrangler.contentPostProcessor) {
                        for (var i=0; i<boardsList.length; i++) {
                            wrangler.contentPostProcessor(boardsList[i], scene);
                        }
                    }
                    resolve(boardsList);
                });
    ...
    ContentWrangler.prototype.addImages = function (sourceUrls, params) {
        ...
        for (var i=0; i<sourceUrls.length; i++) {
            boardsDat.push({
                type: 'image',
                src: 'https://crossorigin.me/'+sourceUrls[i],
                meta_orig: {},
