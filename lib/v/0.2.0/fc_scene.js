/* The machinery of a Scene. */
/* For examples of how to use this, look at the boilerplate. http://meta4vr.net/carnival/boilerplate */
...
window.FCScene = (function () {
    ...
    var Scene = function () {
        ...
        this.sceneObjects = [];
        this.textures = {}; // TODO not true >> /* label: {obj: , ptr: } */
    ...
    /* Gets the player's translation relative to room-space origin */
    /* IE - the location of their head relative to the center of the room */
    Scene.prototype.getPlayerTranslation = function () {
        var pose = window.vrDisplay.getPose();
        var position = vec3.fromValues(pose.position[0], pose.position[1], pose.position[2]);
        var orientation = quat.create();
        var out = mat4.create()
    ...
    /* Variant that only moves the player; and expects the raft to notice, and move itself. */
    Scene.prototype.movePlayerTo = function (loc, params) {
        let t = this.getPlayerTranslation();
        
        this.setPlayerLocation({x:loc.x-t[0], y:loc.y+0.01, z:loc.z-t[2]});
        // this.getObjectByLabel('raft').relocateTo(raftloc);
        // this.setPlayerLocation(raftloc);
    ...
    Scene.prototype.prepareObjectRecursive = function (obj) {
        // if (!obj.isRenderable && obj.drawable) obj = obj.drawable;
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (prereqs.materials) {
                ...
                for (var i=0; i<prereqs.materials.length; i++) {
                    ...
                    else if (matIn.color) matOut.texture = CARNIVAL.color(matIn.color).asTexture();
			...
			if (prereqs.components) {
				prereqs.components.forEach(cspec => {
					prereqPromises.push(cspec.library.load(cspec.globalName, cspec.localName))
				});
			}
            
            /* TODO add components to this */
    ...
    Scene.prototype.prepareObject = function (obj) {
        ...
        if (obj.mesh) {
            ...
            obj.bounds = FCMeshTools.analyseMesh(obj.mesh); /* ... ugh ... */
    ...
    Scene.prototype._renderTemporaryGeometry = function (geomCluster, projectionMat, viewMat, pov, renderState) {
        ...
        */
        ...
        while (geomCluster.length) {
            ...
            // console.log('rendering temp geom', geom);
            window.GEOM = geom;
            ...
            var u = shader.uniform;
            var a = shader.attrib;
            var tex = geom.texture || scene.textures.cyan;
            gl.bindTexture(gl.TEXTURE_2D, tex);
            shader.use();
            gl.uniformMatrix4fv(u.projectionMat, false, projectionMat);
            if (geom.modelMat) mat4.mul(mVM, viewMat, geom.modelMat);
            else mVM = viewMat;
            ...
            gl.vertexAttribPointer(a.position, 3, gl.FLOAT, false, 32, 0); // 3x 4 octets = 12
            gl.vertexAttribPointer(a.texCoord, 2, gl.FLOAT, false, 32, 12); // 12 + 2x 4 octets = 20
            gl.vertexAttribPointer(a.vertexNormal, 3, gl.FLOAT, false, 32, 20); // 20 + 3x 4 octets = 32
    ...
    Scene.prototype._renderObjectFace = function (sO, projectionMat, modelViewMat, pov, renderState) {
        ...
        /* Set up scene lights */
        // console.log(scene.shaders);
        // var ads = scene.shaders.ads;
        // console.log(ads);
        // var u = ads.uniform;
        // scene.gl.uniform3vf(u['material.Ambient'], [1,1,1]);
        // scene.gl.uniform3vf(u['lights[1].Ambient'], [1,1,1]);
    ...
    Scene.prototype.render = function (projectionMat, viewMat, pov) {
            ...
            for (var i=0; i<vrGamepads.length; i++) {
                ...
                if (window.vrDisplay.stageParameters && gPose.orientation && gPose.position) {
                    ...
                    spatial.hands[i] = {
                        ...
                        ori: {x:newOri[0], y:newOri[1], z:newOri[2]} //<<< this is probably totally wrong, haha, because quats
                    };
                }
                else {
        ...
        /* Render temporary geometry */
        if (scene._temporaryGeometry.length) {
            scene._temporaryGeometry = scene._renderTemporaryGeometry(scene._temporaryGeometry, projectionMat, viewMat, pov, renderState);
        }
        ...
        var consumeGraphObject = function (parentObj, childObj) {
            ...
            /* VVV injectedMatrix. This sort of thing is why you need a cookbook or something */
            ...
            if (childObj._temporaryGeometry) childObj._temporaryGeometry = scene._renderTemporaryGeometry(childObj._temporaryGeometry, projectionMat, viewMat, pov, renderState);
