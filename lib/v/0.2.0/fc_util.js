    ...
    */
    var makeTextTexture = function (gl, textBlocks, canvasParams) {
        ...
        for (var i=0; i<textBlocks.length; i++) {
            ...
            console.log('rendering text:', tb.text)
        }
    ...
    var makeTextBoard = function (scene, textBlocks, pos, orientation, size, groupLabel, meta, params) { 
        //<< size and ori should be flipped!!! it's pos/size/ori, always. also groupLabel maybe could be 
        //inorporated into a params-type structure - to make it all more like building a drawable
        size = size | {};
        ...
        return new Promise(function (resolve, reject) {
            ...
            var textTex = renderTextToTexture(scene.gl, textBlocks, {
                ...
                canvasColor: p.canvasColor || 'gray',
                topMargin: 40,
                leftMargin: 40
            ...
            var board = new FCShapes.BoardCuboid(
                ...
                orientation,
                {
                    textureLabel: 'gray', //<< this is a problem 
                    shaderLabel: 'basic',
                    groupLabel: groupLabel || 'contentboards',
                    label: 'bla'
                }
        ...
        var trk = function (drawable, timePoint) {
            ...
            if (vrGamepads.length && vrGamepads[gamepadIndex]) {
                ...
                if (drawable.rotation || drawable.translation) {
                    ...
                    // var qua = drawable.rotationQuaternion;
                    ...
                    drawable.injectedMatrix = finalmat;
                }
...
/* ===== ==== ===== ==== ===== ==== ===== ==== ===== ==== */	
    ...
    var makeType2ButtonHandler = function (gamepadIndex, buttonActions, sectorConfig={}) {
        /* Accepts an array of buttonActions of the form:
			buttonAction = {
				trigger: () => {},
				action: () => {},
				label: '...'
			}
			On each cycle, iterates over this list, testing each trigger, and runs the actions that are appropriate.
			Each trigger gets passed both raw and abstracted gamepad info.
		*/
		let {centerRadius=0.4, sectorCount=12, sectorOffset=0} = sectorConfig;
		
		
		let scratchpadTag = Math.floor(Math.random()*10**10)
		...
		let trackpadState = function (gamepad, drawable) {
            drawable.scratchPad['trackpadAxes'] = gamepad.axes;
			let trackpad = gamepad.buttons[0];
			let sector, segment, angle, radius, touched=trackpad.touched, axes=gamepad.axes;
			let [axisx, axisy] = axes;
            if (touched) {
                var a = -0.5 < axisx < 0.5,
                    b = axisx < -0.5 || axisx > 0.5,
                    c = -0.5 < axisy < 0.5,
                    d = axisy < -0.5 || axisy > 0.5;
				segment = {
					0: 'center',
					1: 'n', 
					5: 'ne',
                ...
                drawable.scratchPad['trackpadSegment'] = segment;

				radius = Math.sqrt(axisx**2 + axisy**2);
				angle = Math.atan2(axisy, axisx);

				let basis = (sectorOffset + angle + Math.PI) % (2*Math.PI);
				let sectwidth = (2*Math.PI)/sectorCount;
				sector = Math.floor(basis / sectwidth);
			}
			return {segment, touched, angle, radius, axes, sector};
			
		}
		
		let buttonState = function (gamepad, button, drawable, btnIdx) {
			let bstatus = {
				down: false, up: false, pressed: false, released: false, held: false, description: null, raw: button
			};
            // let scratchPadKey = 'Button' + btnIdx + 'Down';
			let scratchPadKey = `${scratchpadTag}_Button${btnIdx}Down`;
			// console.log(drawable.scratchPad);
            let prevState = drawable.scratchPad[scratchPadKey] || false;
            let buttonStatus = (button.pressed ? 'down' : 'up');
			bstatus.down = button.pressed;
			bstatus.up = !button.pressed;
			// console.log(`button pressed: ${button.pressed} vs prevState: ${prevState}`)
            if (button.pressed != prevState) {
                buttonStatus = (button.pressed ? 'pressed' : 'released')
                drawable.scratchPad[scratchPadKey] = button.pressed;
				bstatus.pressed = button.pressed;
				bstatus.released = !button.pressed;
				// console.log(`button state changed to ${buttonStatus}`);
				window.XYZZY3 = bstatus;
            }
            else if (button.pressed) {
                buttonStatus = 'held';
				bstatus.held = true;
            }
			bstatus.description = buttonStatus;
			if (btnIdx == 0) {
				Object.assign(bstatus, trackpadState(gamepad, drawable));
			}
			return bstatus;
		}
		
        var handle = function (drawable, timePoint) {
            // console.log('hi!');
            var vrGamepads = getVRGamepads();
            // console.log('Got ', vrGamepads.length, 'VR gamepads from ', gamepads.length, 'total gamepads');
            if (vrGamepads.length && vrGamepads[gamepadIndex]) {
				...
				let gpData = {
					gamepad: myGp,
					buttonsByIndex: myGp.buttons.map((myButton, btnIdx) => {
						return buttonState(myGp, myButton, drawable, btnIdx);
					})
				}
				let gpSupp = {buttons:{}};
				gpData.buttonsByIndex.forEach((myButton, btnIdx) => gpSupp.buttons[viveButtonMap[btnIdx]] = myButton);
				let gpInfo = Object.assign({}, gpData, gpSupp);
				
				buttonActions.forEach(action => {
					if (action.trigger(gpInfo)) {
						action.action(gpInfo)
					}
				});
...
/* ===== ==== ===== ==== ===== ==== ===== ==== ===== ==== */	
    ...
    */
    ...
    PlanarCollider.prototype.findRayCollision = function (testRayOrigin, testRayVec, bracketDist, ctxObj, debugObj) {
        ...
        */
        ...
        if (collider.collisionBoxes.length) {
            ...
            console.log('Collider was empty?');
    ...
    return {
		...
		makeType2ButtonHandler: makeType2ButtonHandler,
