...
importScripts('../../thirdparty/gl-matrix-min.js');
SYNTH_NORMALS = false;
...
var parseOBJ = function (objectData) {
    ...
    // var vv1=null, vv2=null, vv3=null;
    
    for (var i = 0; i < lines.length; i++) {
        if (config.reportProgress && i%20000 == 0) postMessage({status:'progress', type:'parse', value:i/lines.length});
      ...
      if (VERTEX_RE.test(line)) {
        ...
        for (var j = 0, eleLen = elements.length; j < eleLen; j++){
            ...
            if(elements[j] in unpacked.hashindices){
                ...
                // vertex position
                var a = +verts[(vertex[0] - 1) * 3 + 0];
                var b = +verts[(vertex[0] - 1) * 3 + 1];
                var c = +verts[(vertex[0] - 1) * 3 + 2]
                unpacked.verts.push(a);
                unpacked.verts.push(b);
                unpacked.verts.push(c);
                vvx[j] = [a,b,c];
                // console.log(j,vvx);
                // vertex textures
                if (textures.length) {
    ...
    this.getMesh = function (meshUrl) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (config.reportProgress) {
                loader.req.onprogress = function (evt) {
                    if (evt.lengthComputable) {
                        var percentComplete = Math.round(100*(evt.loaded / evt.total));
                        if (percentComplete > loader.lastProgressReport) {
                            postMessage({status:'progress', type:'download', value:percentComplete/100});
                            loader.lastProgressReport = percentComplete;
                        }
                    }
                }
            }
...
var meshData = null;
var loader = null;
var config = null; // reportProgress = [true|false]
